...
export default function QwenImageEditForm({ workspaceId }: { workspaceId?: string }) {
  ...
  const [pricing, setPricing] = useState({ creditQwenImageEdit: 75 });
  const totalCost = images.length * pricing.creditQwenImageEdit;
  ...
  useEffect(() => {
    fetch('/api/credit-pricing').then(r => r.json()).then(d => {
      if (d.success) setPricing(prev => ({ ...prev, creditQwenImageEdit: d.pricing.creditQwenImageEdit || 75 }));
    }).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!taskName.trim()) { alert("Vui lòng nhập tên cho task."); return; }
    if (!prompt.trim()) { alert("Vui lòng nhập mô tả chỉnh sửa (prompt)."); return; }
    ...
    try {
      ...
      const uploadedDriveIds: string[] = uploadData.driveFileIds;

      // Call generate API
      const res = await fetch("/api/generate-qwen-image-edit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          access_token: accessToken,
          imagesToEdit: uploadedDriveIds,
          prompt: prompt.trim(),
          workspaceId,
  ...
  const promptSuggestions = [
    "Thay nền thành bãi biển nhiệt đới, giữ nguyên nội thất",
    "Chuyển sang phong cách anime Ghibli, giữ bố cục",
    "Thêm cây xanh và ánh nắng vào sân vườn",
    "Nâng cấp nội thất sang phong cách hiện đại, sang trọng",
    "Thay đổi ánh sáng thành hoàng hôn ấm áp",
  ];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5 pb-2">
          ...
          <input
            ...
            className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-cyan-500 outline-none text-sm"
            required
          />
          <p className="text-[11px] text-gray-500">Tên này sẽ được dùng làm tên thư mục trên Google Drive.</p>
        </div>

        <div {...getRootProps()} className={`border-2 border-dashed p-8 rounded-2xl flex flex-col items-center justify-center cursor-pointer transition ${isDragActive ? 'border-cyan-500 bg-cyan-50' : 'border-gray-300 bg-gray-50 hover:bg-gray-100'}`}>
          <input {...getInputProps()} />
        ...
        {images.length > 0 && (
          <div className="flex flex-wrap gap-4">
            {images.map((img, index) => (
              <div key={index} className="relative w-24 h-24 bg-gray-100 rounded-lg overflow-hidden group border border-gray-200 shadow-sm">
      ...
      <div className="space-y-4 border-t border-gray-100 pt-4">
        <h2 className="text-xl font-bold text-gray-800 border-b pb-2">Mô tả chỉnh sửa (Prompt)</h2>

        <div className="space-y-2">
          <label className="block text-sm font-semibold text-gray-700">
            Bạn muốn AI chỉnh sửa ảnh như thế nào? <span className="text-red-500">*</span>
          </label>
          <textarea
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
            placeholder="VD: Thay nền thành bãi biển, giữ nguyên căn nhà. Thêm cây xanh vào sân..."
            className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-cyan-500 outline-none text-sm min-h-[100px] resize-y"
            required
          />
          <p className="text-[11px] text-gray-500">Mô tả càng chi tiết, kết quả càng chính xác. Prompt sẽ áp dụng cho tất cả ảnh.</p>
        </div>

        {/* Prompt suggestions */}
        <div>
          <label className="block text-xs font-semibold text-gray-500 mb-2">💡 Gợi ý prompt:</label>
          <div className="flex flex-wrap gap-2">
            {promptSuggestions.map((suggestion, idx) => (
              <button
                ...
                className="text-xs px-3 py-1.5 bg-cyan-50 text-cyan-700 border border-cyan-200 rounded-lg hover:bg-cyan-100 transition cursor-pointer"
              >
                {suggestion}
              </button>
            ))}
      ...
      <div className="pt-2 space-y-5 border-t border-gray-100">
        ...
        <div>
          ...
          <div className="flex items-center space-x-3">
            ...
            {selectedDriveFolder && (
              <div className="flex items-center bg-cyan-50 text-cyan-700 px-3 py-1.5 rounded-lg border border-cyan-100 text-sm">
                <span className="truncate max-w-[200px] font-medium">{selectedDriveFolder.name}</span>
                <button type="button" onClick={() => setSelectedDriveFolder(null)} className="ml-2 hover:bg-cyan-100 rounded-full p-0.5"><X className="w-3.5 h-3.5" /></button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-50 border border-gray-200 p-4 rounded-xl flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {images.length > 0 ? `${images.length} ảnh × ${pricing.creditQwenImageEdit} credits` : 'Chưa có ảnh để xử lý'}
            </p>
          </div>
      ...
      <div className="pt-4">
        {isSuccess ? (
          ...
          <button disabled={loading || images.length === 0 || !prompt.trim() || (accessToken.length > 0 && userCredits < totalCost)} type="submit" className={`w-full text-white font-bold py-3.5 px-4 rounded-xl transition-all flex items-center justify-center space-x-2 disabled:opacity-75 disabled:shadow-none ${accessToken && images.length > 0 && prompt.trim() && userCredits >= totalCost ? 'bg-cyan-600 hover:bg-cyan-700 shadow-[0_4px_14px_0_rgba(8,145,178,0.39)] hover:-translate-y-0.5' : 'bg-gray-400'}`}>
            {loading ? <Loader2 className="animate-spin" /> : <Sparkles size={20} />}
            <span>{loading ? "Đang gửi ảnh..." : (!accessToken ? "Uỷ quyền Google Drive" : (images.length === 0 ? "Chọn ảnh để bắt đầu" : (!prompt.trim() ? "Nhập prompt để tiếp tục" : (userCredits < totalCost ? "Không đủ Credit" : `Chỉnh sửa ${images.length} ảnh ✨`))))}</span>
          </button>
        )}
      </div>
    </form>
