...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!subFolderId || !access_token) {
      console.error("[Webhook-VisionFlux] Missing metadata in URL");
      return NextResponse.json({ error: "Missing metadata" }, { status: 400 });
    }

    // Replicate trả về đối tượng Prediction
    const prediction = await req.json();
    console.log(`[Webhook-VisionFlux] Nhận webhook từ Replicate. Status: ${prediction.status}`);

    if (prediction.status !== "succeeded") {
      return NextResponse.json({ success: true, message: "Bỏ qua vì tiến trình chưa chạy xong / lỗi" });
    }

    // Lấy link kết quả output từ Flux Fill.
    let resultUrl = "";
    ...
    if (!resultUrl) {
      console.error("[Webhook-VisionFlux] Không tìm thấy URL kết quả (Output null?) Cấu trúc output Replicate:", JSON.stringify(prediction.output));
      return NextResponse.json({ error: "No output URL" }, { status: 400 });
    }

    // --- TẢI ẢNH REPLICATE XUỐNG VÀO BUFFER ---
    console.log(`[Webhook-VisionFlux] Lấy ảnh thành quả từ resultURL...`);
    ...
    console.log(`[Webhook-VisionFlux] Đưa vào thư mục Drive của user...`);
    ...
    const finalName = `[AI-VisionFlux] ${Math.random().toString(36).substring(7)}_${originalFileName}`;
    ...
    console.log(`✅ [Webhook-VisionFlux] Thêm ảnh thành công vào Google Drive. ID: ${driveRes.data.id}`);
    return NextResponse.json({ success: true, fileId: driveRes.data.id });

  } catch (error: any) {
    console.error("[Webhook-VisionFlux] Lỗi:", error.message || error);
