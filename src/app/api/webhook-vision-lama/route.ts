...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!subFolderId || !access_token) {
      console.error("[Webhook-VisionLama] Missing metadata in URL");
      return NextResponse.json({ error: "Missing metadata" }, { status: 400 });
    }

    // Replicate trả về đối tượng Prediction
    const prediction = await req.json();
    console.log(`[Webhook-VisionLama] Nhận webhook từ Replicate. Status: ${prediction.status}`);

    if (prediction.status !== "succeeded") {
      return NextResponse.json({ success: true, message: "Bỏ qua vì tiến trình chưa chạy xong / lỗi" });
    }

    // Lấy link kết quả output từ LAMA. Thường LAMA trả về một string url hoặc base64
    let resultUrl = "";
    ...
    if (!resultUrl) {
      console.error("[Webhook-VisionLama] Không tìm thấy URL kết quả (Output null?) Cấu trúc output Replicate trả về:", JSON.stringify(prediction.output));
      return NextResponse.json({ error: "No output URL" }, { status: 400 });
    }

    // --- TẢI ẢNH REPLICATE XUỐNG VÀO BUFFER ---
    console.log(`[Webhook-VisionLama] Lấy ảnh thành quả từ resultURL...`);
    ...
    console.log(`[Webhook-VisionLama] Đưa vào thư mục Drive của user...`);
    ...
    const finalName = `[AI-VisionLama] ${Math.random().toString(36).substring(7)}_${originalFileName}`;
    ...
    console.log(`✅ [Webhook-VisionLama] Thêm ảnh thành công vào Google Drive. ID: ${driveRes.data.id}`);
    return NextResponse.json({ success: true, fileId: driveRes.data.id });

  } catch (error: any) {
    console.error("[Webhook-VisionLama] Lỗi:", error.message || error);
